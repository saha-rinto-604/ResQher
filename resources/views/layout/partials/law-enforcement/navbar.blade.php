...
<header class="header">
    ...
    <div class="left-content d-flex flex-wrap gap-10">
        <!-- Dashboard -->
        <a href="{{url('/')}}" class="btn-primary-fill btn-sm small-btna {{Route::currentRouteName() === 'law-enforcement.dashboard' ? 'active' : ''}}">Dashboard</a>
        <a href="{{route('law-enforcement.documents')}}" class="btn-primary-fill btn-sm small-btna {{Route::currentRouteName() === 'law-enforcement.documents' ? 'active' : ''}}">Documents</a>
    </div>
    <!-- / Left -->
    ...
    <ul class="header-right">
        @php
            $volunteer_details = \Illuminate\Support\Facades\DB::select('select * from lawenforcement_details where user_id = ?', [auth()->user()->id]);
