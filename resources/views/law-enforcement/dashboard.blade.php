    ...
    <script>
        $(document).ready(function () {
            ...
            const getLatestVictims = () => {
                $.ajax({
                    url: "{{ route('law-enforcement.dashboard.latest-victims') }}",
                    type: "GET",
            ...
            function getLocation(victim_id) {
                $.ajax({
                    url: `{{ route('law-enforcement.dashboard.victim-coordinate') }}`,
                    type: 'POST',
                    data: {
                        victim_id: victim_id,
                        _token: '{{ csrf_token() }}'
            ...
            function updateSelfLocation() {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function (position) {
                        ...
                        $.ajax({
                            url: "{{ route('law-enforcement.dashboard.update-self-location') }}",
                            type: "POST",
                            data: {
                                latitude: myLat,
                                longitude: myLng,
