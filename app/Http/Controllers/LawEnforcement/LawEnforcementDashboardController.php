...
class LawEnforcementDashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $user = DB::select('
                                SELECT lawenforcement_details.*, users.id, users.phone
                                FROM lawenforcement_details
                                INNER JOIN users ON users.id = lawenforcement_details.user_id
    ...
    {
        $user_id = auth()->user()->id;
        $self_details = DB::select('SELECT id,availability FROM lawenforcement_details WHERE user_id = ?', [$user_id]);
        if ($self_details) {
        ...
        ');
                ...
                if (!empty($latestVictims)) {
                    $myData = DB::select('SELECT id,latitude,longitude FROM lawenforcement_details WHERE user_id = ?', [auth()->user()->id]);
                    if (empty($myData)) {
                        $myData = null;
                    } else {
                        $myData = $myData[0];
    ...
    public function documents()
    {
        ...
        return view('law-enforcement.documents', [
    ...
    public function uploadDocuments(Request $request)
    {
        $request->validate([
            ...
            'job_id_card' => 'nullable|file|mimes:jpg,jpeg,png|max:2048',
...
//        $phone_number = Str::replaceStart("88", "", $phone_number);
        ...
        $student_id_card = $request->file('job_id_card');
        ...
        if ($student_id_card) {
            $student_id_card_name = time() . '_job_id_card.' . $student_id_card->getClientOriginalExtension();
        }

        $user_id = auth()->user()->id;

        $query = "INSERT INTO lawenforcement_details (user_id, nid_front_side, nid_back_side, job_id_card, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?)
                  ON DUPLICATE KEY UPDATE nid_front_side = ?, nid_back_side = ?, job_id_card = ?, latitude = ?, longitude = ?";
        ...
        if (DB::getPdo()->lastInsertId()) {
            $nid_front_side->move(public_path('assets/uploads/law-enforcement/documents'), $nid_front_side_name);
            $nid_back_side->move(public_path('assets/uploads/law-enforcement/documents'), $nid_back_side_name);

            if ($student_id_card) {
                $student_id_card->move(public_path('assets/uploads/law-enforcement/documents'), $student_id_card_name);
            }
    ...
    {
        $volunteer_details = DB::select('SELECT * FROM lawenforcement_details WHERE user_id = ?', [auth()->user()->id]);
        if ($volunteer_details) {
            $volunteer_details = $volunteer_details[0];
        } else {
            $volunteer_details = null;
        ...
        if ($volunteer_details) {
            ...
            DB::update('UPDATE lawenforcement_details SET availability = ? WHERE user_id = ?', [!$availability, auth()->user()->id]);
    ...
    public function updateSelfLocation(Request $request)
    {
        ...
        DB::update('UPDATE lawenforcement_details SET latitude = ?, longitude = ?, updated_at = NOW() WHERE user_id = ?', [$latitude, $longitude, $user_id]);
